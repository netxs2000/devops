...
def test_submit_bug():
    ...
    data = {
        "requester_name": "张三",
        "requester_email": "zhangsan@example.com",
        "title": "登录页面无法正常显示",
        "severity": "S2",
        "priority": "P2",
        "province": "广东",
        "environment": "production",
        "steps_to_repro": "1. 打开登录页面\n2. 输入用户名和密码\n3. 点击登录按钮",
        "actual_result": "页面显示空白，无法登录",
        "expected_result": "应该正常跳转到主页",
...
def test_submit_requirement():
    ...
    data = {
        "requester_name": "李四",
        "requester_email": "lisi@example.com",
        "title": "增加数据导出功能",
        "description": "希望能够将报表数据导出为 Excel 格式，方便离线分析。\n\n具体需求：\n1. 支持导出当前筛选条件下的数据\n2. 支持自定义导出字段\n3. 支持批量导出",
        "req_type": "feature",
        "priority": "P2",
        "province": "nationwide",
...
def test_list_tickets():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, ticket in enumerate(tickets[:5], 1):  # 只显示前5个
