...
def setup_test_data(db: Session):
    ...
    if not test_user:
        test_user = User(full_name='业务员小张', primary_email='biz_user@example.com', employee_id='BIZ001', department_id=biz_dept.org_id, identity_map={'gitlab': {'username': 'zhangsan_biz'}})
        db.add(test_user)
    else:
    ...
    issue_cross = IssueMetadata(gitlab_project_id=999, gitlab_issue_iid=1, global_issue_id=10001, dept_name='研发部', title='TEST_VISIBILITY_跨部门需求', state='opened', author_username='zhangsan_biz', author_dept_name='业务部', sync_status=1)
    issue_private = IssueMetadata(gitlab_project_id=999, gitlab_issue_iid=2, global_issue_id=10002, dept_name='研发部', title='TEST_VISIBILITY_研发部内部任务', state='opened', author_username='rd_engineer', author_dept_name='研发部', sync_status=1)
    db.add(issue_cross)
...
def run_visibility_test():
    ...
    try:
        ...
        if has_cross and (not has_private):
            ...
            print('❌ 验证失败！')
            if not has_cross:
                print('  - 错误：用户没看到自己提报的工单。')
            if has_private:
                print('  - 错误：隔离泄露，用户看到了研发部内部工单。')
    finally:
        db.close()
