...
"""'''
    ...
    def test_sync_all_jobs(self):
        """测试同步所有 Job 逻辑。"""
        mock_jobs = [{'name': 'job1', 'fullName': 'job1', 'url': 'http://j/job1', 'color': 'blue'}, {'name': 'job2', 'fullName': 'folder/job2', 'url': 'http://j/folder/job2', 'color': 'red'}]
    ...
    def test_sync_job_builds(self):
        ...
        self.session.query().filter_by().first.side_effect = [mock_job, None]
        mock_builds = [{'number': 1, 'url': 'http://j/job1/1'}]
        self.client.get_builds.return_value = mock_builds
        mock_build_detail = {'number': 1, 'url': 'http://j/job1/1', 'result': 'SUCCESS', 'duration': 1000, 'timestamp': 1600000000000, 'building': False, 'actions': [{'_class': 'hudson.model.CauseAction', 'causes': [{'userName': 'admin'}]}]}
