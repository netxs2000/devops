...
def test_sse_connection():
    ...
    try:
        ...
        for line in response.iter_lines():
            if time.time() - start_time > 10:  # 监听10秒
                break
...
def test_quality_gate_notification():
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/projects/{PROJECT_ID}/quality-gate",
            headers={"Authorization": f"Bearer {TOKEN}"}
        )
...
def test_test_execution_notification():
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/projects/{PROJECT_ID}/test-cases/{ISSUE_IID}/execute",
            headers={"Authorization": f"Bearer {TOKEN}"},
            params={"result": "failed"},
            json={"comment": "测试失败：环境配置错误"}
        )
