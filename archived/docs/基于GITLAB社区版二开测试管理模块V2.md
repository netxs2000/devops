...
## 7. 方案演进：基于 FastAPI 的轻量级侧边栏中台 (V2.2 - 推荐方案)

在实际企业环境中，直接修改 GitLab 核心源码（基于 Rails 和 GDK）门槛较高且维护成本大。**方案 V2.2 提出了一种“非侵入式”的侧边栏中台模式**，利用 FastAPI 作为中间层，通过 GitLab API 和 Webhook 实现所有测试管理功能，而无需修改 GitLab 任何一行源代码。

### 7.1 核心理念：Issue-as-Database (议题即数据库)
该方案完全利用 GitLab 原生的 Issues 系统作为存储引擎：
*   **用例存储**：每个测试用例对应一个 Issue，使用 `type::test` 标签区分。
*   **结构化数据**：利用特定的 Markdown 模板（如 `## Preconditions`, `## Steps`）存储用例细节。
*   **状态管理**：利用 `status::passed`, `status::failed`, `status::blocked` 标签标识执行状态。
*   **跨系统联动**：通过 Webhook 捕获事件并触发逻辑处理。
*   **需求关联**：利用 `type::requirement` 标签和特定的 Markdown 引用格式实现用例与原始需求的绑定。

### 7.3 端到端追溯与功能闭环 (End-to-End Traceability)

为了对标 GitLab Ultimate 的核心能力，方案 V2.2 补全了“需求-用例-缺陷”的完整追溯链：

#### 7.3.1 需求管理 (Requirement Management)
*   **需求实体化**：使用带有 `type::requirement` 标签的 Issue 作为业务需求载体。
*   **追溯绑定**：在测试用例的 Markdown 描述中，利用 `关联需求]: # IID` 格式建立显式连接。
*   **覆盖分析**：中台 API 能够自动检索需求下挂载的所有测试用例，并实时计算该需求的“测试覆盖情况”。

#### 7.3.2 自动化状态反馈 (V2.2 进阶)
*   **满足状态追踪**：当一个需求关联的所有测试用例均处于 `passed` 状态时，系统可自动将需求的 Issue 标签更新为 `requirement::satisfied`。
*   **质量红线**：若任一关键测试用例失败，关联需求自动标记为 `requirement::failed`，实现风险的直观可视化。
    
#### 7.3.3 地域质量分析黑科技 (Province Quality Analytics) 🌟 (New)
*   **多维元数据驱动**：TestHub 提单界面已集成“发现省份”字段，提交时自动注入 `province::*` 标签。
*   **地域热力图看板**：在报表中心提供极坐标区域图，实时反映各省份的缺陷集中度，辅助定点治理。
*   **智能语义证言**：自动提取高风险省份，在《版本质量证言》中实现自动化风险告知与决策建议。

### 7.2 后端架构实现 (FastAPI)
我们使用 Python 和 FastAPI 构建了一个轻量级的同步驱动器：

#### 7.2.1 结构化数据解析与转换
后端负责将 GitLab Issue 的 Markdown 描述解析为前端可操作的 JSON 结构，并提供反向更新。

#### 7.2.2 增强审计：执行备注 (Execution Comments)
为了补充社区版缺乏的“执行历史”功能，每次在中台点击执行时，后端会通过 API 在 Issue 下自动提交一条 Notes：
```python
# 核心逻辑示例
comment_body = f"### [TestHub] 执行反馈\n- **结果**: {result}\n- **执行人**: {executor}\n- **流水线**: #{pipeline_id}\n- **时间**: {timestamp}"
gitlab_client.post_issue_comment(project_id, issue_iid, comment_body)
```

### 7.3 端到端功能闭环

#### 7.3.1 缺陷全生命周期联动
*   **一键报障**：执行失败时，中台自动生成预填写的 Bug 创建链接。
*   **缺陷看板**：中台通过 `type::bug` 标签实时汇总当前项目的缺陷分布（Fixed Rate, Open Bugs），构建质控驾驶舱。

#### 7.3.2 CI/CD 深度集成
通过监听 GitLab Webhook 的 `Pipeline Hook`，中台能够实时感知项目的最新代码流向：
1.  **流水线监控**：侧边栏展示当前项目最新的 Pipeline ID、Commit SHA 及运行状态。
2.  **执行自动关联**：测试执行记录会自动挂载当前的 `pipeline_id`，实现“测试结果-代码版本”的精确追溯。

#### 7.3.3 一键导出综合质量报告
系统支持导出整合了测试用例库和缺陷全景的 Markdown 报告，内容包括：
*   **质量仪表盘**：通过率、各模块用例分布。
*   **缺陷全景分析**：详细列出所有存量 Bug 及其修复状态、负责人。
*   **需求覆盖矩阵**：列出每个需求的测试状态，明确哪些需求已验证，哪些仍存在风险点。
*   **执行日志**：详细的测试执行步骤与审计记录。

### 7.4 为什么选择方案 V2.2？
1.  **零侵入**：不影响 GitLab 升级，不依赖 GDK 开发环境。
