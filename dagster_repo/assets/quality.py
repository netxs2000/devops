...
def check_developer_profile(db: DatabaseResource):
    ...
    results = batch.validate(gx.expectations.ExpectColumnValuesToNotBeNull(column='user_id'), gx.expectations.ExpectColumnValuesToBeBetween(column='total_impact_score', min_value=0), gx.expectations.ExpectColumnValuesToBeInSet(column='developer_archetype', value_set=['Review Master', 'Code Machine', 'Task Closer', 'Generalist']))
    return AssetCheckResult(passed=results.success, metadata={'success_percent': results.statistics['success_percent'], 'total_expectations': results.statistics['evaluated_expectations'], 'failed_expectations': results.statistics['unsuccessful_expectations']})
...
def check_dora_metrics(db: DatabaseResource):
    ...
    results = batch.validate(gx.expectations.ExpectColumnValuesToNotBeNull(column='deployment_count'), gx.expectations.ExpectColumnValuesToBeBetween(column='change_failure_rate', min_value=0, max_value=100))
    return AssetCheckResult(passed=results.success, metadata={'success_percent': results.statistics['success_percent']})
...
def check_metrics_guard(db: DatabaseResource):
    ...
    results = batch.validate(gx.expectations.ExpectColumnValuesToBeBetween(column='variance_percentage', min_value=-5, max_value=5))
    return AssetCheckResult(passed=results.success, metadata={'anomalies_detected': int(len(df[df['is_anomaly'] == True])), 'success_percent': results.statistics['success_percent']})