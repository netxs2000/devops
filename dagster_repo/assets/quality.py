...
@asset_check(
    asset=AssetKey(["devops_dbt_assets", "fct_developer_activity_profile"]),
    description="Check for non-null user_id and reasonable impact scores",
)
def check_developer_profile(db: DatabaseResource):
    ...
    results = batch.validate(
        ...
        gx.expectations.ExpectColumnValuesToBeInSet(
            column="developer_archetype", value_set=["Review Master", "Code Machine", "Task Closer", "Generalist"]
        ),
    )
...
@asset_check(
    asset=AssetKey(["devops_dbt_assets", "fct_metrics_audit_guard"]),
    description="Guard against high variance in metrics",
)
