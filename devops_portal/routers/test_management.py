...
async def list_test_cases(project_id: int, current_user=Depends(get_current_user), db: Session=Depends(auth_router.get_db)):
...
async def create_test_case(project_id: int, data: schemas.TestCaseCreate, current_user=Depends(check_permission(['maintainer', 'admin']))):
    """在线录入并创建测试用例。"""
    try:
        service = TestingService()
        issue = await service.create_test_case(project_id=project_id, title=data.title, priority=data.priority, test_type=data.test_type, pre_conditions=data.pre_conditions.split('\n') if isinstance(data.pre_conditions, str) else data.pre_conditions, steps=data.steps, requirement_id=str(data.requirement_iid) if data.requirement_iid else None, creator=current_user.full_name)
...
async def import_test_cases(project_id: int, file: UploadFile=File(...), current_user=Depends(check_permission(['maintainer', 'admin']))):
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            import_items.append({'title': str(row.get('title', 'Untitled')), 'priority': str(row.get('priority', 'P2')), 'test_type': str(row.get('test_type', '功能测试')), 'requirement_id': str(row.get('requirement_id', '')) if not pd.isna(row.get('requirement_id')) else None, 'pre_conditions': str(row.get('pre_conditions', '')).split('\n'), 'steps': steps})
        service = TestingService()
...
async def clone_test_cases(project_id: int, source_project_id: int=Query(...), current_user=Depends(check_permission(['maintainer', 'admin']))):
    """从源项目克隆所有测试用例到当前项目。"""
    try:
        service = TestingService()
...
async def upload_project_file(project_id: int, file: UploadFile=File(...)):
    ...
    try:
        ...
        return {'alt': uploaded_file['alt'], 'url': uploaded_file['url'], 'markdown': uploaded_file['markdown']}
    except Exception as e:
        logger.error(f'Upload failed: {e}')
        raise HTTPException(status_code=500, detail=str(e))

@router.post('/test-cases/generate-from-ac')
async def generate_steps_from_ac(project_id: int, requirement_iid: int=Query(...), current_user=Depends(get_current_user)):
...
async def generate_test_case_from_requirement(project_id: int, iid: int, title: Optional[str]=None, priority: str='P2', test_type: str='功能测试', current_user=Depends(get_current_user)):
    ...
    try:
        ...
        issue = await service.create_test_case(project_id=project_id, title=case_title, priority=priority, test_type=test_type, requirement_id=str(iid), pre_conditions=[], steps=steps, creator=current_user.full_name)
        return {'status': 'success', 'issue': issue}
...
async def generate_and_create_test_case_from_description(project_id: int, title: Optional[str]=None, priority: str='P2', test_type: str='功能测试', description: str=Body(..., embed=True), current_user=Depends(get_current_user)):
    ...
    try:
        ...
        issue = await service.create_test_case(project_id=project_id, title=case_title, priority=priority, test_type=test_type, requirement_id=None, pre_conditions=[], steps=steps, creator=current_user.full_name)
        return {'status': 'success', 'issue': issue}
    except HTTPException:
        raise
    except Exception as e:
...
async def generate_steps_from_description(project_id: int, description: str=Body(..., embed=True), current_user=Depends(get_current_user)):
...
async def generate_test_code_from_requirement(project_id: int, iid: int, title: Optional[str]=None, priority: str='P2', test_type: str='功能测试', current_user=Depends(get_current_user)):
    ...
    try:
        ...
        temp_tc = portal_schemas.TestCase(id=0, iid=0, title=case_title, priority=priority, test_type=test_type, requirement_id=str(iid), pre_conditions=[], steps=[portal_schemas.TestStep(step_number=i + 1, action=s.get('action', ''), expected_result=s.get('expected', '')) for i, s in enumerate(steps)], result='pending', web_url='', linked_bugs=[])
...
@router.get('/test-cases/deduplication-report')
async def scan_for_duplicates(project_id: int, type: str='requirement', current_user=Depends(get_current_user)):
    """[AI] 精准检测项目中语义重复的工单组。"""
    service = TestingService()
    ...
    return {'groups': clusters, 'clusters': clusters, 'saving_potential': f'{saving_potential}%', 'total_groups': len(clusters)}

@router.post('/test-cases/{iid}/generate-code')
async def generate_automation_code(project_id: int, iid: int, request: Request, current_user=Depends(get_current_user)):
...
async def create_requirement(project_id: int, data: schemas.RequirementCreate, current_user=Depends(get_current_user)):
    """创建新的需求。"""
    try:
        service = TestingService()
        result = await service.create_requirement(project_id=project_id, title=data.title, priority=data.priority, category=data.req_type, business_value=data.description, acceptance_criteria=[], creator_name=current_user.full_name)
        return result
    except Exception as e:
...
async def update_requirement_review_state(project_id: int, iid: int, review_state: str, current_user=Depends(get_current_user)):
    """更新需求的评审状态。"""
    ...
    try:
        ...
        for issue in issues:
            ...
            if 'type::bug' in labels or 'bug' in labels:
                bugs.append(schemas.BugDetail(iid=issue.iid, title=issue.title, state=issue.state, created_at=datetime.fromisoformat(issue.created_at.replace('Z', '+00:00')), author=issue.author['name'], web_url=issue.web_url, labels=labels))
        return bugs
    except Exception as e:
...
@router.post('/defects')
async def create_defect(project_id: int, data: schemas.BugCreate, current_user=Depends(get_current_user)):
    """QA 专业缺陷提报接口。"""
    try:
        service = TestingService()
        result = await service.create_defect(project_id=project_id, title=data.title, severity=data.severity, priority=data.priority, category=data.category, env=data.environment, steps=data.steps_to_repro, expected=data.expected_result, actual=data.actual_result, reporter_name=current_user.full_name, related_test_case_iid=data.linked_case_iid)
        return result
    except Exception as e:
...
async def export_rtm_report(project_id: int, current_user=Depends(get_current_user), db: Session=Depends(auth_router.get_db)):
    ...
    try:
        ...
        return Response(content=md, media_type='text/markdown', headers={'Content-Disposition': f'attachment; filename=RTM_Report_P{project_id}.md'})
    except Exception as e:
...
async def execute_test_case(project_id: int, issue_iid: int, result: str=Query(None), report: Optional[schemas.ExecutionReport]=None, current_user=Depends(check_permission(['tester', 'maintainer', 'admin']))):
    ...
    try:
        ...
        record = schemas.ExecutionRecord(issue_iid=issue_iid, result=final_result, executed_at=datetime.now(), executor=executor, executor_uid=executor_uid, comment=comment, pipeline_id=PIPELINE_STATUS.get(project_id, {}).get('id'))
        ...
        return {'status': 'success', 'new_result': final_result, 'history': EXECUTION_HISTORY[issue_iid][:5]}
    except Exception as e:
...
async def export_test_report(project_id: int):
    ...
    try:
        ...
        return PlainTextResponse(report_md, headers={'Content-Disposition': f'attachment; filename=quality_report_p{project_id}.md'})
    except Exception as e:
...
async def get_bug_report_link(project_id: int, issue_iid: int):
    ...
    try:
        ...
        description = f'### 🛡️ Test Failure Report\\n\\n- **Target Case**: #{issue_iid} ({issue.web_url})\\n- **Detected At**: {datetime.now().isoformat()}\\n- **Reproduction**: See steps in linked test case.\\n\\n### 📝 Additional Context\\nAutomatically generated via QA Hub.'
        params = {'issue[title]': title, 'issue[description]': description, 'add_labels': 'type::bug,status::confirmed'}
...
async def get_requirement_stats(project_id: int, current_user=Depends(get_current_user), db: Session=Depends(auth_router.get_db)):
    ...
    try:
        ...
        if approved_count == 0:
            return schemas.RequirementCoverage(total_count=total_count, approved_count=0, covered_count=0, passed_count=0)
        details = await asyncio.gather(*[service.get_requirement_detail(project_id, r.iid) for r in approved_reqs])
        covered_count = 0
        passed_count = 0
        risk_reqs = []
