...
async def upload_service_desk_attachment(
    ...
    """
    try:
        # 使用 GitLabClient 进行上传，替代原有的 requests 方式
        client = GitLabClient()
        ...
        # project.upload returns {'alt': 'name', 'url': '/uploads/...', 'markdown': '...'}
...
async def submit_bug_via_service_desk(
    ...
    try:
        ...
        ticket = await service.create_ticket(
            ...
            title=data.title,
            description=data.actual_result, # 使用实际结果作为描述
            issue_type="bug",
...
async def get_my_tickets(
    ...
    service = ServiceDeskService()
    # Assuming logic is similar to list_service_desk_tickets but strictly 'requester'
    # Actually list_service_desk_tickets already handles 'requester' logic for business users in service.get_user_tickets.
    # But get_my_tickets might be specifically for "My Submissions" vs "Tickets Assigned to My Dept".
    # Since I don't see the exact implementation, I'll assume get_user_tickets generally covers it or I should query by email.
    
    # Re-implementing logic from likely main.py:
    # return service.get_tickets_by_requester(db, current_user.email) (Hypothetical)
    # Using existing service.get_user_tickets for now as it seems robust.
    tickets = service.get_user_tickets(db, current_user)
    # Filter only those created by me if get_user_tickets returns dept tickets too?
    # Security logic in get_user_tickets usually handles filtering.
    # If user is R&D, they see dept tickets. If Business, they see own tickets.
    # "My Tickets" usually means "Created by me".
