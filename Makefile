...
install: ## [内用] 安装生产环境或开发环境依赖
	...
	$(EXEC_CMD) bash -c "pip install --default-timeout=5 -i http://192.168.1.168:8081/repository/pypi-all/simple --trusted-host 192.168.1.168 . || \
		pip install --default-timeout=30 . || \
...
init-dev: ## [本地] 初始化开发环境依赖 (Nexus优先)
	...
	python -m pip install --upgrade pip -i http://192.168.1.168:8081/repository/pypi-all/simple --trusted-host 192.168.1.168 || python -m pip install --upgrade pip || python -m pip install --upgrade pip -i https://pypi.tuna.tsinghua.edu.cn/simple
	pip install -e .[dev] -i http://192.168.1.168:8081/repository/pypi-all/simple --trusted-host 192.168.1.168 || pip install -e .[dev] || pip install -i https://pypi.tuna.tsinghua.edu.cn/simple -e .[dev]
...
lock: ## [工具] 将 pyproject.toml 的依赖锁定到 requirements.txt
	...
	$(EXEC_CMD) bash -c "pip-compile pyproject.toml -o requirements.txt --resolver=backtracking --no-header -i http://192.168.1.168:8081/repository/pypi-all/simple --trusted-host 192.168.1.168 || \
...
e2e-install: ## [本地] 安装 Playwright E2E 测试依赖
	...
	pip install -e .[e2e] -i http://192.168.1.168:8081/repository/pypi-all/simple --trusted-host 192.168.1.168 || \
