...
install: ## [内用] 安装生产环境或开发环境依赖
	...
	$(EXEC_CMD) bash -c "pip install --no-build-isolation --default-timeout=5 -i http://192.168.5.64:8082/repository/group-pypi/simple --trusted-host 192.168.5.64 .[dev,test] || \
		pip install --no-build-isolation --default-timeout=30 .[dev,test] || \
		pip install --no-build-isolation -i https://pypi.tuna.tsinghua.edu.cn/simple --default-timeout=60 .[dev,test]"
