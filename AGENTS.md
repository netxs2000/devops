...
1.  **终极验证律 (The Law of Verify)**：所有 L2 及以上任务完成前，**必须执行** `make verify`。
    - **硬性标准**：代码覆盖率 (Coverage) 必须 **>= 80%**，否则禁止交付。
    - **操作要求**：必须在回复中粘贴 `make verify` 的终端输出日志碎片，作为物理取证。
2.  **容器强制验证**：所有涉及后端逻辑的变更必须通过容器内测试验证。**严禁**以“本地 Windows 跑通”作为 DoD 标准。
3.  **环境自愈清场**：离场前强制执行 `make clean`。**严禁**在根目录残留任何调试脚本、临时日志或导出文件。
4.  **安全审计律 (The Law of Security)**：涉及 core/mdm 核心表变更或引入新依赖的 [L3/L4] 任务，**必须执行** `make security-audit`。
    - **标准**：必须通过 TruffleHog (无机密泄露) 与 Trivy (无 HIGH/CRITICAL 镜像漏洞)。
    - **操作要求**：必须在交付报告中包含安全审计摘要。
5.  **绝对取证原则**：进度汇报必须包含控制台输出碎片。**严禁**出现“逻辑预测认为代码正确”的幻觉。
