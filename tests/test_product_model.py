...
class TestProductModel(unittest.TestCase):
    ...
    def test_product_hierarchy_and_roles(self):
        ...
        # 4. Create Product under Product Line
        prod = Product(
            name="Mobile App",
            level="Product",
            parent_id=pline.id,
            product_line_name=pline.name,
            organization_id=center.id,
            product_manager_id=pm.id,
            dev_manager_id=dev.id
        )
