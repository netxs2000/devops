...
def test_project_master_creation(db_session):
    ...
    db_session.add(org)
    pm = ProjectMaster(project_id='PRJ_TEST_001', project_name='测试主项目', project_type='SPRINT', status='PLAN', org_id='DEPT_IT_001', budget_code='B_2026_01', budget_type='CAPEX')
    db_session.add(pm)
    db_session.commit()
