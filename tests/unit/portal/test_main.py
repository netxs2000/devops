...
def test_list_jenkins_jobs(authenticated_client, db_session):
    # Setup data
    job = JenkinsJob(name="test-job", full_name="test-job", url="http://jenkins/job/test-job")
    db_session.add(job)
    db_session.commit()
...
def test_list_jenkins_builds(authenticated_client, db_session):
    ...
    build = JenkinsBuild(job_id=job.id, number=1, result="SUCCESS", url="http://jenkins/job/test-job/1")
...
def test_gitlab_webhook(client):
    payload = {
        "object_kind": "issue",
        "object_attributes": {"id": 123, "iid": 1, "action": "update", "state": "opened"},
...
def test_list_dependency_scans(authenticated_client, db_session):
    project = GitLabProject(id=123, name="test-project", path_with_namespace="test/project")
    db_session.add(project)
    db_session.commit()

    scan = DependencyScan(project_id=project.id, scan_status="completed", scanner_name="OWASP Dependency-Check")
    db_session.add(scan)
    db_session.commit()
