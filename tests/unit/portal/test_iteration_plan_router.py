...
def test_list_iteration_projects(authenticated_client, db_session):
    project = GitLabProject(id=101, name="Test Project", path_with_namespace="group/test-project")
...
def test_list_milestones(authenticated_client, db_session):
    ...
    milestone = GitLabMilestone(id=202, project_id=101, title="v1.0.0", state="active")
...
def test_trigger_release(authenticated_client, mock_iteration_service):
    ...
    payload = {"version": "v1.0.0", "new_title": "v1.0.0-final", "ref_branch": "main"}
