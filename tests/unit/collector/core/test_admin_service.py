...
def test_create_team(admin_service, db_session):
    """单元测试：验证虚拟团队创建。"""
    payload = schemas.TeamCreate(name="Backend Team", team_code="BE_TEAM")
...
def test_add_team_member(admin_service, db_session):
    ...
    payload = schemas.TeamMemberCreate(user_id=user_id, role_code="DEVELOPER", allocation_ratio=0.5)
