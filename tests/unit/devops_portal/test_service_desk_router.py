...
async def test_submit_requirement(authenticated_client, db_session, mock_gitlab_client):
    ...
    payload = {"title": "Test Req", "description": "I want feature X", "priority": "P2", "attachments": []}
...
def test_track_service_desk_ticket(authenticated_client, db_session):
    ...
    ticket = ServiceDeskTicket(gitlab_project_id=10, gitlab_issue_iid=1, title="Track Me", status="opened")
    db_session.add(ticket)
    db_session.commit()
...
async def test_update_ticket_status(authenticated_client, db_session, mock_gitlab_client):
    ...
    ticket = ServiceDeskTicket(gitlab_project_id=10, gitlab_issue_iid=1, title="Update Me", status="opened")
    db_session.add(ticket)
    db_session.commit()
