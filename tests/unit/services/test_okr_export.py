...
def test_export_okrs_content(admin_service, db_session):
    ...
    org = Organization(org_code="TEST-ORG", org_name="测试组织", org_level=1, is_current=True)
    ...
    kr1 = OKRKeyResult(objective_id=obj.id, title="关键结果1", target_value=100.0, current_value=60.0, progress=0.6, metric_unit="个")
    db_session.add(kr1)
    db_session.commit()
