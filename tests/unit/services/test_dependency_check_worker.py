...
class TestDependencyCheckWorker(unittest.TestCase):
    ...
    def test_process_ci_report_success(self):
        ...
        self.worker._save_dependencies = MagicMock(return_value={"total": 10, "vulnerable": 2, "high_risk_licenses": 1})
    ...
    def test_detect_package_manager(self):
        """测试包管理器检测"""
        test_cases = [
            ({"filePath": "/path/to/pom.xml"}, "maven"),
            ({"filePath": "/path/to/package.json"}, "npm"),
            ({"filePath": "/path/to/requirements.txt"}, "pypi"),
            ({"filePath": "/path/to/project.csproj"}, "nuget"),
            ({"filePath": "/path/to/go.mod"}, "go"),
        ]
    ...
    def test_analyze_vulnerabilities(self):
        """测试漏洞分析"""
        vulnerabilities = [
            {"name": "CVE-2023-12345", "severity": "CRITICAL", "cvssv3": {"baseScore": 9.8}},
            {"name": "CVE-2023-67890", "severity": "HIGH", "cvssv3": {"baseScore": 7.5}},
            {"name": "CVE-2023-11111", "severity": "MEDIUM", "cvssv3": {"baseScore": 5.0}},
        ]
