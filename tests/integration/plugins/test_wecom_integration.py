...
from devops_collector.models import Base, IdentityMapping, Organization, RawDataStaging, User
...
def mock_wecom_client():
    ...
    all_users = []
    seen = set()
    for _dept_id, user_list in MOCK_USERS.items():
        for u in user_list:
...
def wecom_worker(db_session, mock_wecom_client):
    """提供 WeComWorker 实例。"""
    return WeComWorker(session=db_session, client=mock_wecom_client, correlation_id="test-wecom-sync-001")
...
def test_wecom_full_sync_flow(wecom_worker, db_session):
    ...
    # 检查新建用户 (admin_wecom)
    admin_mapping = db_session.query(IdentityMapping).filter_by(source_system="wecom", external_user_id="admin_wecom").first()
    ...
    tester1_mapping = db_session.query(IdentityMapping).filter_by(external_user_id="tester1").first()
