...
class TestTraceabilityModels(unittest.TestCase):
    ...
    def test_traceability_link_storage(self):
        ...
        link = TraceabilityLink(source_system='jira', source_type='issue', source_id='PROJ-101', target_system='gitlab', target_type='mr', target_id='15', link_type='fixes', raw_data={'note': 'Manual link from UI'})
    ...
    def test_gitlab_traceability_fields(self):
        ...
        self.session.flush()
        mr = MergeRequest(id=101, iid=10, project_id=project.id, title='Fix bug for Jira-123', external_issue_id='JIRA-123', issue_source='jira')
        self.session.add(mr)
        commit = Commit(id='sha123456', project_id=project.id, message='Implement feature X, resolves #456 and #789', linked_issue_ids=['456', '789'], issue_source='zentao')
    ...
    def test_jenkins_traceability_fields(self):
        ...
        self.session.flush()
        build = JenkinsBuild(job_id=job.id, number=42, gitlab_mr_iid=15, artifact_id='registry.example.com/app:v1.2.3', artifact_type='docker_image')
