...
from devops_collector.models import Base, Project, Issue, MergeRequest, Commit, GitLabGroup, SyncLog, User, SonarProject, SonarMeasure, JiraProject, JiraIssue, ZenTaoProduct, ZenTaoIssue, NexusComponent, JFrogArtifact
...
"""'''
    ...
    with open(path, 'r', encoding='utf-8') as f:
        return json.load(f)
MOCK_DATA = {'gl_projects': load_mock_json('gitlab_projects.json'), 'gl_issues': load_mock_json('gitlab_issues.json'), 'gl_mrs': load_mock_json('gitlab_mrs.json'), 'gl_groups': load_mock_json('gitlab_group.json'), 'sq_gate': load_mock_json('sonarqube_quality_gate.json'), 'sq_measures': load_mock_json('sonarqube_measures.json'), 'jk_jobs': load_mock_json('jenkins_jobs.json'), 'jk_builds_list': load_mock_json('jenkins_builds_list.json'), 'jk_build_detail': load_mock_json('jenkins_build_detail.json'), 'jr_projects': load_mock_json('jira_projects.json'), 'jr_boards': load_mock_json('jira_boards.json'), 'jr_sprints': load_mock_json('jira_sprints.json'), 'jr_issues': load_mock_json('jira_issues.json'), 'jr_users': load_mock_json('jira_users.json'), 'jr_groups': load_mock_json('jira_groups.json'), 'zt_products': load_mock_json('zentao_products.json'), 'zt_plans': load_mock_json('zentao_plans.json'), 'zt_stories': load_mock_json('zentao_stories.json'), 'nx_repos': load_mock_json('nexus_repositories.json'), 'nx_comps': load_mock_json('nexus_components.json'), 'jf_artifacts': load_mock_json('jfrog_artifacts.json'), 'jf_stats': load_mock_json('jfrog_stats.json'), 'jf_xray': load_mock_json('jfrog_xray.json')}
...
"""'''
    ...
    if 'mock-gitlab.com' in url:
        ...
        if '/repository/commits' in url:
            return MockResponse([])
        if '/issues' in url:
            return MockResponse(MOCK_DATA['gl_issues'])
        if '/merge_requests' in url:
            return MockResponse(MOCK_DATA['gl_mrs'])
        ...
        if 'projects/101' in url:
            return MockResponse(MOCK_DATA['gl_projects'][0])
        if 'groups/10' in url:
            return MockResponse(MOCK_DATA['gl_groups'][0])
        return MockResponse([])
    ...
    if 'mock-jira.com' in url:
        if '/rest/api/3/groups/picker' in url:
            return MockResponse(MOCK_DATA['jr_groups'])
        if '/rest/api/3/users/search' in url:
            return MockResponse(MOCK_DATA['jr_users'])
        if '/rest/api/3/project/PROJ' in url:
            return MockResponse(MOCK_DATA['jr_projects'][0])
        if '/rest/agile/1.0/board' in url and 'sprint' not in url:
            return MockResponse(MOCK_DATA['jr_boards'])
        if '/rest/agile/1.0/board/1/sprint' in url:
            return MockResponse(MOCK_DATA['jr_sprints'])
        if '/rest/api/3/search' in url:
            return MockResponse(MOCK_DATA['jr_issues'])
        return MockResponse({})
    if 'mock-zentao.com' in url:
        if url.endswith('/products'):
            return MockResponse({'products': MOCK_DATA['zt_products']})
        if '/products/1/plans' in url:
            return MockResponse(MOCK_DATA['zt_plans'])
        if '/products/1/stories' in url:
            return MockResponse(MOCK_DATA['zt_stories'])
        if '/products/1/bugs' in url:
            return MockResponse({'bugs': [], 'total': 0})
        if '/products/1/cases' in url:
            return MockResponse({'cases': []})
        if '/products/1/releases' in url:
            return MockResponse({'releases': []})
        if '/products/1/actions' in url:
            return MockResponse({'actions': []})
        if '/products/1' in url:
            return MockResponse(MOCK_DATA['zt_products'][0])
        if '/executions' in url:
            return MockResponse({'executions': []})
        if '/users' in url:
    ...
    if 'mock-nexus.com' in url:
        if '/components' in url:
            return MockResponse(MOCK_DATA['nx_comps'])
        if '/repositories' in url:
            return MockResponse(MOCK_DATA['nx_repos'])
        return MockResponse({})
    if 'mock-jfrog.com' in url:
        if 'stats' in url:
            return MockResponse(MOCK_DATA['jf_stats'])
        if 'xray/summary' in url:
            return MockResponse(MOCK_DATA['jf_xray'])
        return MockResponse({})
...
"""'''
    ...
    session = Session()
    with patch('requests.get', side_effect=mocked_requests_get), patch('requests.post', side_effect=mocked_requests_post):
        ...
        jk_worker.process_task({'job_type': 'sync_builds', 'job_full_name': 'frontend-build'})
