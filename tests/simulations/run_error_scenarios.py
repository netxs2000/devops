...
def run_transient_failure_test():
    ...
    with patch('requests.get', side_effect=side_effect), \
         patch('tenacity.wait_exponential.__call__', return_value=0.1): 
...
def run_rate_limit_test():
    ...
    def side_effect(*args, **kwargs):
        ...
        if call_count == 1:
            ...
            return MockResponse({}, status_code=429, headers={'Retry-After': '0'}) # 0 to speed up test
        return MockResponse({"id": 101, "name": "Test Project", "path_with_namespace": "test/proj"})
