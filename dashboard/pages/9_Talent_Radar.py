...
with k1:
    st.markdown(
        f'<div class="glass-card"><p>Top Talents</p><p class="metric-value">{len(filtered_df[filtered_df["talent_influence_index"] > 50])}</p></div>',
        unsafe_allow_html=True,
    )
with k2:
    specialists = len(filtered_df[filtered_df["talent_archetype"] == "Domain Specialist"])
    st.markdown(
        f'<div class="glass-card" style="border-left: 4px solid #a855f7;"><p>Domain Specialists</p><p class="metric-value" style="background: linear-gradient(90deg, #a855f7, #ec4899); -webkit-background-clip: text;">{specialists}</p></div>',
        unsafe_allow_html=True,
    )
with k3:
    avg_domains = round(filtered_df["metric_knowledge_domains"].mean(), 1)
    st.markdown(
        f'<div class="glass-card"><p>Avg Knowledge Domains</p><p class="metric-value">{avg_domains}</p></div>',
        unsafe_allow_html=True,
    )
with k4:
    risk_count = len(df_risk)
    st.markdown(
        f'<div class="glass-card" style="border-left: 4px solid #ef4444;"><p>Bus Factor Risks</p><p class="metric-value" style="background: linear-gradient(90deg, #ef4444, #f87171); -webkit-background-clip: text;">{risk_count}</p></div>',
        unsafe_allow_html=True,
    )
...
with col2:
    ...
    for _, row in top_talents.iterrows():
        ...
        if row["talent_archetype"] == "Domain Specialist":
            arch_class = "arch-specialist"
        elif row["talent_archetype"] == "Collaborative Leader":
            arch_class = "arch-leader"
        elif row["talent_archetype"] == "Reliable Contributor":
            arch_class = "arch-contributor"
