...
# --- 模拟人工费率配置 (请根据您的财务标准修改) ---
# 单位建议：人天 (Daily Rate)
LABOR_RATES = [
    {"level": "P1/T1", "daily_rate": 800.0,  "desc": "初级 (Junior)"},
    {"level": "P2/T2", "daily_rate": 1200.0, "desc": "中级 (Middle)"},
    {"level": "P3/T3", "daily_rate": 1800.0, "desc": "高级 (Senior)"},
    {"level": "P4/T4", "daily_rate": 2500.0, "desc": "资深 (Expert)"},
    {"level": "P5/T5", "daily_rate": 3500.0, "desc": "架构师/专家 (Principal)"},
    {"level": "M1",    "daily_rate": 2000.0, "desc": "基层管理 (Manager)"},
    {"level": "M2",    "daily_rate": 3000.0, "desc": "中层管理 (Director)"}
]
...
def init_labor_rates():
    ...
    try:
        ...
        for rate_data in LABOR_RATES:
            ...
            # 查找或创建
            obj = session.query(LaborRateConfig).filter(
                LaborRateConfig.job_title_level == level,
                LaborRateConfig.is_active == True
            ).first()
            ...
            obj.daily_rate = rate
            obj.hourly_rate = rate / 8.0 # 假设每日 8 小时
            obj.currency = "CNY"
