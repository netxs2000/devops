...
def test_province_data_isolation():
    ...
    # 测试配置
    BASE_URL = "http://localhost:8001"  # TestHub 服务地址
    PROJECT_ID = 1  # 测试项目ID
    
    # 模拟两个不同权限的用户Token（需要先通过 /auth/login 获取）
    test_users = {
        "nationwide_admin": {
            "description": "全国权限管理员",
            "expected_provinces": ["nationwide", "guangdong", "beijing", "shanghai"],  # 应该能看到所有省份
            "token": "REPLACE_WITH_ACTUAL_TOKEN"  # 需要用真实Token替换
        },
        "guangdong_user": {
            "description": "广东省份用户",
            "expected_provinces": ["guangdong"],  # 只应该看到广东
            "token": "REPLACE_WITH_ACTUAL_TOKEN"  # 需要用真实Token替换
        }
    }
    ...
    for user_type, user_config in test_users.items():
        ...
        # 测试 1: province-quality API
        print(f"\n  测试API: /projects/{PROJECT_ID}/province-quality")
        try:
            response = requests.get(
                f"{BASE_URL}/projects/{PROJECT_ID}/province-quality",
        ...
        # 测试 2: province-benchmarking API
        print(f"\n  测试API: /projects/{PROJECT_ID}/province-benchmarking")
        try:
            response = requests.get(
                f"{BASE_URL}/projects/{PROJECT_ID}/province-benchmarking",
                headers={"Authorization": f"Bearer {token}"}
            )
