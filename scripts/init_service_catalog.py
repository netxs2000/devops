...
# --- 配置您的服务目录 (请在此处修改) ---
# 每个服务可以映射 1 个或多个 GitLab 项目路路径 (path_with_namespace)
SERVICE_CATALOG = [
    {
        "name": "用户中心 (User Center)",
        "tier": "P0",
        "org_name": "研发中心/核心业务部",
        "description": "负责全站用户账号、OAuth 鉴权及基础信息管理。",
        "projects": ["platform/user-api", "platform/user-service"],
        "slos": [
            {"name": "可用性 (Availability)", "type": "Availability", "target": 99.9, "unit": "%"},
            {"name": "响应延迟 (Latency P99)", "type": "Latency", "target": 200, "unit": "ms"}
        ]
    },
    {
        "name": "订单系统 (Order System)",
        "tier": "P0",
        "org_name": "研发中心/核心业务部",
        "description": "负责交易下单、状态机流转及库存扣减逻辑。",
        "projects": ["order-center/order-manager"],
        "slos": [
            {"name": "可用性 (Availability)", "type": "Availability", "target": 99.95, "unit": "%"}
        ]
    }
]
...
def init_catalog():
    ...
    try:
        ...
        for s_data in SERVICE_CATALOG:
            ...
            # 4. 初始化默认 SLO
            # 首先清理旧 SLO (生产环境慎重，此处为初始化逻辑)
            session.query(SLO).filter(SLO.service_id == service.id).delete()
            for slo_data in s_data.get("slos", []):
                new_slo = SLO(
                    service_id=service.id,
                    name=slo_data["name"],
                    indicator_type=slo_data["type"],
                    target_value=slo_data["target"],
                    metric_unit=slo_data["unit"],
                    time_window="28d"
                )
                session.add(new_slo)
