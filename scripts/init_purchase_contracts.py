...
# --- 模拟采购合同数据 ---
SAMPLE_PURCHASE_CONTRACTS = [
    {
        "contract_no": "PUR-2025-AWS-001",
        "title": "AWS 核心生产环境年度认购",
        "vendor_name": "Amazon Web Services",
        "vendor_id": "VEND_AWS_001",
        "total_amount": 1200000.0, # 120万
        "start_date": datetime(2025, 1, 1),
        "end_date": datetime(2025, 12, 31),
        "cost_code_match": "1002.01", # 关联: 云计算节点
        "capex_opex": "OPEX"
    },
    {
        "contract_no": "PUR-2025-OUT-102",
        "title": "2025年度交付外包框架合同",
        "vendor_name": "某动力信息技术有限公司",
        "vendor_id": "VEND_SOFT_102",
        "total_amount": 3000000.0, # 300万
        "start_date": datetime(2025, 1, 1),
        "end_date": datetime(2025, 12, 31),
        "cost_code_match": "1001.03", # 关联: 研发外包费
        "capex_opex": "OPEX"
    }
]
...
def init_purchase_contracts():
    ...
    try:
        ...
        for data in SAMPLE_PURCHASE_CONTRACTS:
            ...
            # 3. [可选逻辑] 生成 1 月份的待摊成本流水示例
            # 在实际系统中，这通常由专门的摊销任务自动生成
            period = "2025-01"
            cost_record = session.query(ResourceCost).filter(
                ResourceCost.purchase_contract_id == pc.id,
                ResourceCost.period == period
            ).first()
            
            if not cost_record:
                # 简单线性摊销逻辑（假设按 12 个月摊销）
