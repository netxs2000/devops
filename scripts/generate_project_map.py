            ...
            for root, dirs, files in os.walk(full_path):
                ...
                for file in files:
                    if file.endswith((".py", ".sh", ".sql", ".md")) and "__init__" not in file:
                        # 核心职责标注逻辑
                        role = ""
                        if "client" in file: role = " [⚡ CLIENT: API Connection]"
                        elif "worker" in file: role = " [⚙️ WORKER: Business Orchestration]"
                        elif "service" in file: role = " [🛠️ SERVICE: Data Logic]"
                        elif "models" in file: role = " [📄 MODELS: Schema]"
                        elif "test_" in file: role = " [🚀 TEST: Unit Protection]"
