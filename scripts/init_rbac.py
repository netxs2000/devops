...
def init_permissions(session):
    """初始化原子权限点。"""
    permissions = [{'code': 'SYSTEM:ADMIN', 'category': 'SYSTEM', 'desc': '系统超级管理权限，无视规则'}, {'code': 'ITERATION:VIEW', 'category': 'ITERATION', 'desc': '查看迭代'}, {'code': 'ITERATION:MANAGE', 'category': 'ITERATION', 'desc': '创建/编辑/规划迭代'}, {'code': 'RELEASE:EXECUTE', 'category': 'ITERATION', 'desc': '执行版本发布'}, {'code': 'USER:VIEW', 'category': 'USER', 'desc': '查看用户档案'}, {'code': 'USER:MANAGE', 'category': 'USER', 'desc': '管理用户角色与归属'}, {'code': 'ANALYTICS:VIEW_ALL', 'category': 'ANALYTICS', 'desc': '查看全公司效能报表'}]
...
def create_super_admin(session):
    ...
    if not user:
        ...
        user = User(global_user_id=uuid.uuid4(), full_name='Super Admin', primary_email=admin_email, is_active=True, is_survivor=True, sync_version=1, is_current=True, is_deleted=False)
        session.add(user)
        session.flush()
        pw_hash = pwd_context.hash(default_password)
        cred = UserCredential(user_id=user.global_user_id, password_hash=pw_hash, last_login_at=None)
        session.add(cred)
        admin_role = session.query(Role).filter_by(code='SYSTEM_ADMIN').first()
        if admin_role:
