...
class IssueMixin:
    ...
    def _sync_issue_events(self, project: GitLabProject, issue_data: Dict) -> None:
        ...
        Args:
            project (GitLabProject): 关联的 GitLabProject 对象。
            issue_data (Dict): GitLabIssue 的详情字典 (必须包含 id 和 iid)。
        """
    ...
    def _save_issue_event(self, issue_id: int, event_type: str, data: Dict) -> None:
        ...
        Args:
            issue_id (int): 数据库中对应的 GitLabIssue 内部 ID。
    ...
    def _sync_issue_flow_details(self, project: GitLabProject, issue_data: Dict) -> None:
        ...
        Args:
            project (GitLabProject): 关联的 GitLabProject 数据库模型实例。
            issue_data (Dict): 从 GitLab API 获取的原始 GitLabIssue 详情数据字典。
        """
        issue_id = issue_data['id']
        issue_iid = issue_data['iid']
        ...
        for event in state_events:
            ...
            if not existing:
                trans = GitLabIssueStateTransition(issue_id=issue_id, from_state=last_state, to_state=current_state, timestamp=current_time, duration_hours=duration)
        ...
        for event in label_events:
            ...
            if action == 'add':
                ...
                last_block = self.session.query(GitLabBlockage).filter(GitLabBlockage.issue_id == issue_id, GitLabBlockage.end_time == None).order_by(GitLabBlockage.start_time.desc()).first()
                if last_block:
                    last_block.end_time = event_time

    def _sync_test_case_from_issue(self, project: GitLabProject, issue: GitLabIssue, data: dict) -> None:
        """从 GitLabIssue 描述解析并同步测试用例。
        
        Args:
            project: 关联的项目。
            issue: 已转换的 GitLabIssue 对象。
            data: 原始 GitLabIssue 数据。
        """
        parsed = GitLabTestParser.parse_description(issue.description or '')
        test_case = self.session.query(GTMTestCase).filter_by(project_id=project.id, iid=issue.iid).first()
        if not test_case:
            test_case = GTMTestCase(project_id=project.id, iid=issue.iid, author_id=issue.author_id)
        ...
        if req_iid:
            req_issue = self.session.query(GitLabIssue).filter_by(project_id=project.id, iid=req_iid).first()
