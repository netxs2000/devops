...
class AirbyteGitLabClient(BaseClient):
    ...
    def get_project_commits(self, project_id: int, since: str | None = None) -> Generator[dict, None, None]:
        """兼容接口：获取项目提交。"""
    ...
    def get_project_issues(self, project_id: int, since: str | None = None) -> Generator[dict, None, None]:
        """兼容接口：获取项目 Issue。"""
        for record in self.get_stream_records("issues"):
            if str(record.get("project_id")) == str(project_id):
                yield record

    def get_project_merge_requests(self, project_id: int, since: str | None = None) -> Generator[dict, None, None]:
        """兼容接口：获取项目 MR。"""
