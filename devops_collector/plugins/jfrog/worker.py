...
class JFrogWorker(BaseWorker):
    ...
    def _save_artifact(self, repo: str, data: dict) -> JFrogArtifact:
        ...
        self.save_to_staging(source='jfrog', entity_type='artifact', external_id=f'{repo}:{path}/{name}', payload=data, schema_version=self.SCHEMA_VERSION)
    ...
    def _sync_xray_scan(self, artifact: JFrogArtifact, scan_data: dict) -> None:
        ...
        self.save_to_staging(source='jfrog', entity_type='xray_scan', external_id=f'artifact_{artifact.id}', payload=scan_data, schema_version=self.SCHEMA_VERSION)
