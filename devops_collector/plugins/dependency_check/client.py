...
class DependencyCheckClient:
    ...
    def scan_project(self, project_path: str, output_dir: str, project_name: Optional[str]=None, scan_transitive: bool=True) -> str:
        """
        扫描项目依赖
        ...
        cmd = [self.cli_path, '--scan', project_path, '--format', 'JSON', '--out', str(output_path), '--prettyPrint']
        if project_name:
        ...
        try:
            result = subprocess.run(cmd, timeout=self.timeout, capture_output=True, text=True, check=False)
