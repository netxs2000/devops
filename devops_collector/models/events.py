...
def auto_link_user_activities(mapper, connection, target):
    ...
    if target.source_system == 'gitlab':
        ...
        # 1. 自动关联 GitLab Commit
        # 匹配规则：Email 匹配 或 用户名匹配
        connection.execute(
            Commit.__table__.update().
            where(
                (Commit.__table__.c.gitlab_user_id == None) & 
                (
                    (Commit.__table__.c.author_email == user.primary_email) | 
                    (Commit.__table__.c.author_name == target.external_username)
                )
            ).
            values(gitlab_user_id=target.global_user_id)
        )
        
        # 2. 自动关联 GitLab Issue (作为作者)
        # 匹配规则：通过 GitLab 内部 UID (存储在 external_user_id)
        if target.external_user_id and str(target.external_user_id).isdigit():
            ...
            connection.execute(
                ...
                where(
                    (Issue.__table__.c.author_id == None) &
                    (func.json_extract(Issue.__table__.c.raw_data, '$.author.id').cast(Integer) == gitlab_uid)
                ).
