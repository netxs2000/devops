...
def _get_plugin_configs(source: str) -> dict:
    ...
    """
    configs = {'gitlab': {'client': {'url': Config.GITLAB_URL, 'token': Config.GITLAB_PRIVATE_TOKEN, 'rate_limit': Config.REQUESTS_PER_SECOND}, 'worker': {'enable_deep_analysis': Config.ENABLE_DEEP_ANALYSIS}}, 'sonarqube': {'client': {'url': Config.SONARQUBE_URL, 'token': Config.SONARQUBE_TOKEN, 'rate_limit': Config.REQUESTS_PER_SECOND}, 'worker': {'sync_issues': Config.SONARQUBE_SYNC_ISSUES}}, 'jenkins': {'client': {'url': Config.JENKINS_URL, 'token': Config.JENKINS_TOKEN, 'user': Config.JENKINS_USER}, 'worker': {'build_limit': Config.JENKINS_BUILD_SYNC_LIMIT}}}
...
def process_task(ch, method, properties, body):
    ...
    try:
        ...
        session = Session()
        worker = PluginRegistry.get_worker_instance(source, session, client, **plugin_cfg['worker'])
        if not worker:
