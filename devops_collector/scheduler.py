        ...
        try:
            ...
            for proj in projects:
                ...
                if should_sync and proj.sync_status != 'SYNCING':
                    task = {'source': 'gitlab', 'project_id': proj.id, 'job_type': 'incremental' if proj.last_synced_at else 'full'}
            ...
            for sp in sonar_projects:
                ...
                if should_sync and sp.sync_status != 'SYNCING':
                    task = {'source': 'sonarqube', 'project_key': sp.key, 'job_type': 'full', 'sync_issues': Config.SONARQUBE_SYNC_ISSUES}
                    mq.publish_task(task)
            ...
            try:
                ...
                result = subprocess.run(['dbt', 'run', '--project-dir', 'dbt_project', '--profiles-dir', 'dbt_project'], capture_output=True, text=True)
                if result.returncode == 0:
                    logger.info('dbt run success')
                    from .core.reverse_etl import sync_talent_tags_to_mdm, sync_aligned_entities_to_mdm, sync_shadow_it_findings
