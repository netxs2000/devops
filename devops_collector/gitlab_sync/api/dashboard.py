...
def get_issue_summary(
    ...
    stats = db.query(
        IssueMetadata.state,
        func.count(IssueMetadata.id)
    ).filter(
        IssueMetadata.id.in_(secure_query.with_entities(IssueMetadata.id))
    ).group_by(IssueMetadata.state).all()
    
    status_map = {s: count for s, count in stats}
    
    # 3. 统计类型分布
    type_stats = db.query(
        IssueMetadata.issue_type,
        func.count(IssueMetadata.id)
    ).filter(
        IssueMetadata.id.in_(secure_query.with_entities(IssueMetadata.id))
    ).group_by(IssueMetadata.issue_type).all()
    ...
    priority_stats = db.query(
        ...
        IssueMetadata.id.in_(secure_query.with_entities(IssueMetadata.id))
    ).group_by(IssueMetadata.priority).all()

    return {
        "summary": {
            "total": sum(status_map.values()),
            "opened": status_map.get("opened", 0),
            "closed": status_map.get("closed", 0)
        },
        "by_type": {t: count for t, count in type_stats if t},
        "by_priority": {p: count for p, count in priority_stats if p}
    }
...
def get_recent_issues(
    ...
    return [
        {
            "gitlab_project_id": issue.gitlab_project_id,
            "iid": issue.gitlab_issue_iid,
            "title": issue.title,
            "state": issue.state,
            "dept_name": issue.dept_name,
            "updated_at": issue.gitlab_updated_at.isoformat() if issue.gitlab_updated_at else None
