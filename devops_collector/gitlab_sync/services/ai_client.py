...
"""'''
    ...
    async def chat(self, prompt: str, system_msg: str='你是一个专业的软件质量保证 (QA) 专家和资深测试工程师。') -> str:
        ...
        payload = {'model': self.model, 'messages': [{'role': 'system', 'content': system_msg}, {'role': 'user', 'content': prompt}], 'temperature': 0.3}
        try:
    ...
    def _get_fallback_response(self, prompt: str) -> str:
        """如果 AI 未配置或失效，提供模拟回复。"""
        if '验收标准' in prompt:
            return json.dumps([{'action': '打开关联的功能页面并检查初始状态', 'expected': '页面加载正常，无报错'}, {'action': '按照验收标准执行核心业务路径输入', 'expected': '系统响应符合预期逻辑，数据正确保存'}, {'action': '验证边界值和负向测试场景', 'expected': '系统能够正确处理异常并提供友好提示'}], ensure_ascii=False)
