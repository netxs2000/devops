    ...
    def sync_issue(self, db: Session, gitlab_issue_data: dict, project_id: int):
        ...
        for label in labels:
            ...
            if label in ['requirement', 'task', 'bug']:
                issue_type = label
        db_issue = db.query(IssueMetadata).filter_by(gitlab_project_id=project_id, gitlab_issue_iid=issue_iid).first()
        if not db_issue:
            db_issue = IssueMetadata(gitlab_project_id=project_id, gitlab_issue_iid=issue_iid, global_issue_id=gitlab_issue_data['id'])
