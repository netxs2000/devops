...
"""'''
    ...
    def parse_markdown_to_test_case(self, issue_data: Dict[str, Any]) -> schemas.TestCase:
        ...
        priority_match = re.search('用例优先级\\]: \\[(P\\d)', desc)
        priority = priority_match.group(1) if priority_match else 'P2'
        type_match = re.search('测试类型\\]: \\[(.*?)\\]', desc)
        test_type = type_match.group(1) if type_match else '功能测试'
        req_match = re.search('关联需求\\]: # (\\d+)', desc)
        req_id = req_match.group(1) if req_match else None
        pre_conditions = []
        if '## 🛠️ 前置条件' in desc:
            try:
                pre_part = desc.split('## 🛠️ 前置条件')[1].split('---')[0]
                pre_conditions = re.findall('- \\[ \\] (.*)', pre_part)
            except IndexError:
                pass
        steps = []
        step_actions = re.findall('\\d+\\. \\*\\*操作描述\\*\\*: (.*)', desc)
        expected_results = re.findall('\\d+\\. \\*\\*反馈\\*\\*: (.*)', desc)
        for i, action in enumerate(step_actions):
            steps.append({'step_number': i + 1, 'action': action, 'expected_result': expected_results[i] if i < len(expected_results) else '无'})
        result = 'pending'
        for label in labels:
            if label.startswith('test-result::'):
                result = label.split('::')[1]
                break
        return schemas.TestCase(id=issue_data['id'], iid=issue_data['iid'], title=issue_data['title'], priority=priority, test_type=test_type, requirement_id=req_id, pre_conditions=[p.strip() for p in pre_conditions], steps=steps, result=result, web_url=issue_data['web_url'], linked_bugs=[])
    ...
    async def get_test_cases(self, db: Session, project_id: int, current_user: Any) -> List[schemas.TestCase]:
        ...
        query = db.query(IssueMetadata).filter(IssueMetadata.gitlab_project_id == project_id, IssueMetadata.issue_type == 'test')
        if not is_admin:
            if allowed_depts:
    ...
    async def execute_test_case(self, project_id: int, issue_iid: int, result: str, executor: str) -> bool:
        ...
        note_body = f"### ✅ Execution Audit Record\n- **Result**: {result.upper()}\n- **Executor**: {executor}\n- **Time**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n--- \n*Generated by TestHub Service*"
        try:
            issue.labels = new_labels
            issue.notes.create({'body': note_body})
            issue.save()
            from devops_portal.main import push_notification
            await push_notification(user_ids='ALL', message=f'Test Case #{issue_iid} updated to {result}', type='refresh_dashboard', metadata={'project_id': project_id, 'iid': issue_iid, 'result': result})
            return True
    ...
    def extract_bugs_from_description(self, description: str) -> List[Dict[str, str]]:
        """从描述中提取关联的缺陷 IID。"""
        bug_matches = re.findall('(?:Bug|缺陷|Fixed by|Related to)\\]?: #(\\d+)', description)
    ...
    async def batch_import_test_cases(self, project_id: int, items: List[Dict[str, Any]]) -> Dict[str, Any]:
        """批量导入测试用例集。
        ...
        tasks = []
        for item in items:
            tasks.append(self.create_test_case(project_id=project_id, title=item.get('title', 'Imported Case'), priority=item.get('priority', 'P2'), test_type=item.get('test_type', '功能测试'), requirement_id=item.get('requirement_id'), pre_conditions=item.get('pre_conditions', []), steps=item.get('steps', [])))
        results = await asyncio.gather(*tasks, return_exceptions=True)
        for i, result in enumerate(results):
            if result and (not isinstance(result, Exception)):
                success_count += 1
            else:
                failed_items.append({'index': i, 'error': str(result)})
        return {'total': len(items), 'success': success_count, 'failed': len(failed_items), 'details': failed_items}

    async def create_defect(self, project_id: int, title: str, severity: str, priority: str, category: str, env: str, steps: str, expected: str, actual: str, reporter_name: str, related_test_case_iid: Optional[int]=None, attachments: Optional[List[str]]=None) -> Dict[str, Any]:
        """为 QA 创建专业缺陷，支持附件。"""
        project = self.get_project(project_id)
        if not project:
        ...
        issue = project.issues.create({'title': f'[BUG] {title}', 'description': description, 'labels': ['type::bug', f'severity::{severity}', f'priority::{priority}', f'bug-category::{category}', 'status::confirmed']})
        ...
        from devops_portal.main import push_notification
        await push_notification(user_ids='ALL', message=f'New Bug reported: {title}', type='refresh_dashboard', metadata={'project_id': project_id, 'iid': issue.iid, 'type': 'bug'})
        return {'iid': issue.iid, 'web_url': issue.web_url, 'message': 'Defect reported successfully'}

    async def create_requirement(self, project_id: int, title: str, priority: str, category: str, business_value: str, acceptance_criteria: List[str], creator_name: str, attachments: Optional[List[str]]=None) -> Dict[str, Any]:
        ...
        issue = project.issues.create({'title': f'[REQ] {title}', 'description': req_description, 'labels': ['type::requirement', f'priority::{priority}', 'status::open']})
        from devops_portal.main import push_notification
        await push_notification(user_ids='ALL', message=f'New Requirement created: {title}', type='refresh_dashboard', metadata={'project_id': project_id, 'iid': issue.iid, 'type': 'requirement'})
        return {'id': issue.id, 'iid': issue.iid, 'web_url': issue.web_url, 'message': 'Requirement created successfully'}
        return clusters
...
"""'''
        ...
        for i in range(len(issues)):
            if issues[i].id in visited:
                continue
            group = [issues[i]]
            visited.add(issues[i].id)
            for j in range(i + 1, len(issues)):
                if issues[j].id in visited:
                    continue
            ...
            if len(group) > 1:
                clusters.append({'prime': {'iid': group[0].iid, 'title': group[0].title, 'url': group[0].web_url}, 'duplicates': [{'iid': g.iid, 'title': g.title, 'url': g.web_url} for g in group[1:]], 'confidence': 0.85})
        return clusters
    ...
    async def reject_ticket(self, project_id: int, ticket_iid: int, reason: str, actor_name: str) -> bool:
        ...
        issue.save()
        from devops_portal.main import push_notification
        await push_notification(user_ids='ALL', message=f'Ticket #{ticket_iid} has been rejected by {actor_name}', type='warning')
        return True
    ...
    async def clone_test_cases_from_project(self, source_project_id: int, target_project_id: int) -> Dict[str, Any]:
        ...
        issues = source_project.issues.list(labels=['type::test'], state='opened', get_all=True)
        if not issues:
        ...
        for issue in issues:
            ...
            import_items.append({'title': tc_data.title, 'priority': tc_data.priority, 'test_type': tc_data.test_type, 'requirement_id': tc_data.requirement_id, 'pre_conditions': tc_data.pre_conditions, 'steps': [{'action': s.action, 'expected': s.expected_result} for s in tc_data.steps]})
    ...
    def generate_test_code_from_case(self, tc_data: schemas.TestCase) -> str:
        ...
        code_lines = ['import pytest', 'from playwright.sync_api import Page, expect', '', f'# Case ID: #{tc_data.iid}', f'# Title: {tc_data.title}', f'# Priority: {tc_data.priority}', '', f'def test_case_{tc_data.iid}(page: Page):', '    """自动化脚本生成于 TestHub AI Engine"""']
    ...
    async def create_test_case(self, project_id: int, title: str, priority: str, test_type: str, requirement_id: Optional[str], pre_conditions: List[str], steps: List[Dict[str, str]], creator: Optional[str]=None) -> Optional[Any]:
        """在线录入并创建测试用例。
        ...
        if not project:
            return None
        description_lines = [f'# 🧪 测试用例: {title}', '', '---', '', '## ℹ️ 基本信息', f'- **用例优先级**: {priority}', f'- **测试类型**: {test_type}', f"- **关联需求**: # {(requirement_id if requirement_id else '[未关联]')}", '', '---', '', '## 🛠️ 前置条件']
        for pc in pre_conditions:
            description_lines.append(f'- [ ] {pc}')
        description_lines.extend(['', '---', '', '## 📝 测试步骤'])
        for i, step in enumerate(steps):
            description_lines.append(f"{i + 1}. **操作描述**: {step.get('action', '')}")
        description_lines.extend(['', '---', '', '## ✅ 预期结果'])
        for i, step in enumerate(steps):
            description_lines.append(f"{i + 1}. **反馈**: {step.get('expected', '')}")
        description_lines.extend(['', '---', '', '## 🚀 执行记录 (Execution Result)', '> **操作说明**: 测试执行完成后，请在下方勾选结论，并**复制对应指令到评论区执行**。', '', '- [ ] **✅ 通过 (Pass)**: `/label ~"test-result::passed" /close` ', '- [ ] **❌ 失败 (Fail)**: `/label ~"test-result::failed"` ', '- [ ] **⚠️ 阻塞 (Blocked)**: `/label ~"test-result::blocked"` ', '', '---', '', '/label ~"type::test" ~"status::todo"'])
        try:
            issue = project.issues.create({'title': title, 'description': '\n'.join(description_lines), 'labels': ['type::test', 'status::todo']})
    ...
    async def generate_quality_report(self, project_id: int) -> str:
        ...
        report = [f'# 📊 质量评估证言 (Quality Testimony) - {project.name}', f"> **Reported On**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", '', '---', '', '## 📈 核心交付指标 (Core Metrics)', '| 指标项 | 当前数据 | 状态 |', '| :--- | :--- | :--- |', f"| **基准用例通过率** | `{pass_rate:.1f}%` | {('✅ 达标' if pass_rate >= 80 else '⚠️ 预警')} |", f"| **未关闭缺陷 (Bugs)** | `{len(bugs)}` 个 | {('🆗 正常' if len(bugs) < 5 else '🔥 高危')} |", f'| **需求基准链条** | `{len(reqs)}` 条 | 📋 已入库 |', '', '---', '', '## 🛡️ 版本发布合规性分析 (Compliance Analysis)', f'- **测试覆盖性**: 总计执行了 `{total_tests}` 个关键场景。', f'- **风险拦截**: 当前共有 `{failed_tests}` 个功能模块处于 Blocking 状态。' if failed_tests > 0 else '- **风险拦截**: 目前无阻塞性漏洞，核心链路验证通过。', '', '---', '', '## 💡 改进建议 (Next Steps)', '1. ' + ('针对失败用例，请研发人员立即介入排查证据链。' if failed_tests > 0 else '当前版本可进入预发布阶段，建议补充性能基准测试。'), '2. 保持对 Service Desk 反馈的实时跟进。', '', '---', '*Generated by TestHub Enterprise Intelligence Platform*']
        return '\n'.join(report)
    ...
    async def analyze_defect_root_cause(self, project_id: int, current_issue_iid: int) -> Dict[str, Any]:
        ...
        historical_bugs = project.issues.list(labels=['type::bug'], state='closed', per_page=50)
        if not historical_bugs:
            return {'suggestions': ['暂无相似历史案例，建议进行首次根因审计。'], 'impact_scope': 'Unknown', 'similar_cases': []}
        similar_cases = []
        for old_bug in historical_bugs:
            ...
            if score > 0.45:
                ...
                similar_cases.append({'iid': old_bug.iid, 'title': old_bug.title, 'solution': solution[:100] + '...', 'score': score})
        suggestions = []
        if similar_cases:
        ...
        return {'title': current_issue.title, 'suggestions': suggestions, 'impact_scope': '建议回归该模块关联的测试套件', 'similar_cases': sorted(similar_cases, key=lambda x: x['score'], reverse=True)[:3]}
    ...
    async def mark_associated_tests_as_stale(self, project_id: int, requirement_iid: int):
        ...
        test_issues = project.issues.list(labels=['type::test'], search=search_query, state='opened')
        for test in test_issues:
            labels = test.labels
            if 'status::stale' not in labels:
                labels.append('status::stale')
                test.notes.create({'body': f'⚠️ **系统治理通知**: 关联的需求 #{requirement_iid} 已发生内容变更，本用例可能已失效。请 QA 团队及时评估并更新测试逻辑。'})
                test.labels = labels
                test.save()
                logger.info(f'Marked test case #{test.iid} as STALE due to req #{requirement_iid} change')
                from devops_portal.main import push_notification
                await push_notification(user_ids='STAKEHOLDERS', message=f'Requirement #{requirement_iid} changed. Associated test cases have been flagged.', type='warning')
    ...
    async def generate_steps_from_requirement(self, project_id: int, requirement_iid: int) -> Dict[str, Any]:
        ...
        if not ac_list:
            return {'title': issue.title, 'ac_found': 0, 'steps': [{'action': '手动检查需求描述', 'expected': '根据描述自行设计测试步骤'}], 'warning': "未在描述中探测到 '## 验收标准' 或结构化列表。"}
        steps = await self.ai.generate_steps_from_ac(issue.title, ac_list)
        return {'title': issue.title, 'ac_found': len(ac_list), 'steps': steps}
        ...
        for issue in filtered_issues:
            ...
            for label in labels:
                if label.startswith('review-state::'):
                    ...
                    break
            reqs.append(portal_schemas.RequirementSummary(iid=issue['iid'], title=issue['title'], state=issue['state'], review_state=review_state))
        return reqs
        ...
        try:
            ...
            for label in req_data.get('labels', []):
                if label.startswith('review-state::'):
                    review_state = label.split('::')[1]
                    break
            test_issues = project.issues.list(labels=['type::test'], search=f'#{iid}', get_all=True)
            test_cases = [self.parse_markdown_to_test_case(issue.attributes) for issue in test_issues]
            return portal_schemas.RequirementDetail(id=req_data['id'], iid=req_data['iid'], title=req_data['title'], description=req_data.get('description'), state=req_data['state'], review_state=review_state, test_cases=test_cases)
        except Exception as e:
