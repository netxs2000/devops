...
class IdentityManager:
    ...
    def get_or_create_user(session: Session, source: str, external_id: str, email: Optional[str]=None, name: Optional[str]=None, employee_id: Optional[str]=None) -> User:
        ...
        user = None
        if email:
            user = session.query(User).filter_by(primary_email=email.lower(), is_current=True).first()
            if user:
        ...
        if not user:
            ...
            user = User(global_user_id=new_global_id, full_name=name or (email.split('@')[0] if email else f'{source}_{external_id}'), primary_email=email.lower() if email else None, employee_id=employee_id, is_active=True, is_survivor=True, sync_version=1, is_current=True, is_deleted=False)
            session.add(user)
            session.flush()
            logger.info(f'Created new global user: {user.full_name} (ID: {new_global_id})')
        if not mapping:
            mapping = IdentityMapping(global_user_id=user.global_user_id, source_system=source, external_user_id=str(external_id), external_username=name, external_email=email.lower() if email else None)
            session.add(mapping)
            session.flush()
        return user