...
def sync_talent_tags_to_mdm(session: Session, threshold: float=50.0) -> int:
    ...
    for row in results:
        ...
        if current_user:
            ...
            new_data = {'raw_data': new_tags, 'sync_version': current_user.sync_version}
            try:
                close_current_and_insert_new(session, User, {'global_user_id': user_id}, new_data)
                updated_count += 1
            except Exception as e:
...
def sync_shadow_it_findings(session: Session) -> int:
    ...
    for row in results:
        ...
        if not existing:
            new_issue = ComplianceIssue(issue_type='SHADOW_IT', severity='HIGH' if row['shadow_it_status'] == 'HIGH_RISK_SHADOW' else 'MEDIUM', entity_id=repo_id, description=f"发现影子系统：仓库 '{row['project_name']}' (ID: {repo_id}) 活跃度高但未在 MDM 注册。", metadata_payload={'status_label': row['shadow_it_status'], 'activity_count': row['last_30d_activity_count']})
