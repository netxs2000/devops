...
class OrganizationService:
    ...
    def upsert_organization(
        self, org_code: str, org_name: str, org_level: int = 1, parent_org_code: str | None = None, manager_raw_id: str | None = None, source: str = "unknown"
    ) -> Organization:
        """Phase 1: 幂等同步部门基本信息。
        ...
        """
        org = self.session.query(Organization).filter_by(org_code=org_code, is_current=True).first()
        ...
        if parent_org_code:
            parent = self.session.query(Organization).filter_by(org_code=parent_org_code, is_current=True).first()
            if parent:
    ...
    def get_org_by_code(self, org_code: str) -> Organization | None:
        """按外部代码查找部门。"""
        return self.session.query(Organization).filter_by(org_code=org_code, is_current=True).first()

    def get_full_hierarchy(self, root_code: str | None = None):
        """获取全量拓扑树 (用于前端视图展现)。"""
