...
def get_business_linked_roles(db: Session, user_id: Any) -> List[str]:
    ...
    managed_gitlab_groups = db.query(GitLabGroupMember.id).filter(
        GitLabGroupMember.user_id == user_id,
        GitLabGroupMember.access_level >= 40
    ).all()
    if managed_gitlab_groups:
        roles.append(BUSINESS_ROLE_MAP['org_manager'])

    # GitLab 项目负责人映射为项目经理
    managed_gitlab_projects = db.query(GitLabProjectMember.id).filter(
        GitLabProjectMember.user_id == user_id,
        GitLabProjectMember.access_level >= 40
    ).all()
...
def get_dynamic_permissions(db: Session, user_id: Any) -> Set[str]:
    ...
    permissions = db.query(SysMenu.perms).join(
        ...
        SysRole, SysRole.id == SysRoleMenu.role_id
    ).filter(
        SysRole.role_key.in_(dynamic_roles),
        SysMenu.perms != None,
        SysMenu.perms != '',
        SysMenu.status == True
    ).all()
