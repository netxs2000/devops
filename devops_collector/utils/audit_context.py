...
# 1. 核心操作者 (Who)
actor_id_ctx: ContextVar[Optional[UUID]] = ContextVar("actor_id_ctx", default=None)
actor_name_ctx: ContextVar[Optional[str]] = ContextVar("actor_name_ctx", default=None)

# 2. 外部环境 (Where)
client_ip_ctx: ContextVar[Optional[str]] = ContextVar("client_ip_ctx", default=None)
request_id_ctx: ContextVar[Optional[str]] = ContextVar("request_id_ctx", default=str(uuid.uuid4()))

# 3. 业务链路 (Context)
correlation_id_ctx: ContextVar[Optional[str]] = ContextVar("correlation_id_ctx", default=None)
audit_remark_ctx: ContextVar[Optional[str]] = ContextVar("audit_remark_ctx", default=None)

def set_audit_user(actor_id: Optional[UUID] = None, actor_name: Optional[str] = None):
    """绑定当前协程的操作者身份。"""
    if actor_id:
...
def set_audit_metadata(client_ip: Optional[str] = None, correlation_id: Optional[str] = None, remark: Optional[str] = None):
    """绑定操作相关的元数据环境。"""
    if client_ip:
