...
#### 2.1.11 企业级 RBAC 2.0 权限体系 🚀 (New)

系统内置了工业级的 RBAC 2.0 权限引擎，实现了从“功能权限”到“行级数据权限”的深度管控：

* **核心五表模型**: 基于 `sys_role`, `sys_menu`, `sys_user_roles`, `sys_role_menu`, `sys_role_dept` 构成的标准化架构，确保权限逻辑与业务模型解耦。
* **树形菜单管理**: 支持无限极菜单目录、功能菜单及按钮操作符的统一管理，权限标识符采用 `业务:模块:操作` (如 `system:user:list`) 的通用规范。
* **角色继承 (Role Hierarchy)**: 支持父子角色关联（RBAC1），子角色可自动继承父角色的权限集，极大降低了大规模组织下的配置开销。
* **多维行级数据隔离 (RLS - Row Level Security)**:
  * **自动化 SQL 注入过滤**: 系统通过 `data_scope` 属性自动在查询层注入过滤条件。
  * **五大权限范围**: 支持“全部数据”、“自定义部门 (通过 `sys_role_dept` 关联)”、“本部门”、“本部门及以下 (递归组织树)”、“仅本人。
* **超级管理员特权**: `SYSTEM_ADMIN` 角色享有全局通配符权限 (`*`)，自动绕过所有卫兵拦截。
* **JWT 动态同步**: Token 载荷中实时注入用户的角色、聚合后的权限标识符列表以及最优 `data_scope` 等信息，支持前端组件的动态显隐。
